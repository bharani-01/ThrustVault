...
async function login(req, res) {
  ...
  try {
    ...
    // ── Cognito timeout fallback: verify password locally in database ───────────────────────────────
    if (/timeout|connect|endpoint/i.test(msg)) {
      try {
        ...
        console.error('[Login offline fallback]', dbErr.message);
        return res.status(400).json({ error: 'Database authentication failed' });
      }
    }
