...
# ---------------------------------------------------------------------------
def extract_host_from_supabase_url(url):
    if not url:
        return None
    try:
        ...
        print(f"Failed to extract host from Supabase URL '{url}': {e}")
        return None
...
DB_HOST = os.environ.get("DB_HOST") or extract_host_from_supabase_url(os.environ.get("SUPABASE_URL"))
DB_PORT = os.environ.get("DB_PORT") or "6543"  # default PgBouncer port for Supabase
DB_NAME = os.environ.get("DB_NAME") or "postgres"
DB_USER = os.environ.get("DB_USER") or "postgres"
DB_PASSWORD = os.environ.get("DB_PASSWORD")

AUDIT_DB_HOST = os.environ.get("AUDIT_DB_HOST") or extract_host_from_supabase_url(os.environ.get("AUDIT_SUPABASE_URL"))
AUDIT_DB_PORT = os.environ.get("AUDIT_DB_PORT") or "6543"
...
if DB_HOST and DB_PASSWORD:
    try:
        ...
        print(f"ADMIN DATABASE CONNECTION WARNING: Failed to initialize pool to {DB_HOST}. Falling back to REST API. Error: {e}")
else:
...
if AUDIT_DB_HOST and AUDIT_DB_PASSWORD:
    try:
        ...
        print(f"ADMIN AUDIT DATABASE CONNECTION WARNING: Failed to initialize pool to {AUDIT_DB_HOST}. Falling back to REST API. Error: {e}")
else:
...
def call_supabase_api_http(path, method='GET', payload=None, query_params=None, headers=None, is_auth=False):
    ...
    try:
        ...
        print(f"Supabase HTTP request failed: {e.code} - {err_body}")
        try:
...
def call_supabase_api(path, method='GET', payload=None, query_params=None, headers=None, is_auth=False):
    if is_auth:
        return call_supabase_api_http(path, method, payload, query_params, headers, is_auth=True)
        
    if db_pool is not None:
        try:
            return execute_sql_on_pool(db_pool, path, method, payload, query_params)
        except Exception as e:
            print(f"Direct SQL query failed on '{path}' ({method}): {e}. Falling back to REST HTTP API.")
            
    return call_supabase_api_http(path, method, payload, query_params, headers, is_auth=False)
...
def post_log_to_supabase(payload):
    if audit_db_pool is not None:
        try:
            ...
            print("Direct SQL audit log insert failed. Falling back to REST API. Error:", e)
    ...
    try:
        ...
        req = urllib.request.Request(
            f"{audit_url}/rest/v1/audit_logs",
...
def auth_login():
    ...
    try:
        # Check if this is a demo account bypass to avoid Cognito timeouts in sandbox/development
        is_demo_bypass = email.endswith('@thrustvault.in') and password == 'ThrustVault123!'
        ...
        if is_demo_bypass:
            ...
            if profile_res and len(profile_res) > 0:
                ...
                user_id = profile_res[0].get('id', 'mock_admin_uid')
                session['email'] = email
                session['role'] = role
                session['uid'] = user_id
                import secrets
        ...
        # OFFLINE/SANDBOX FALLBACK: If Cognito fails/times out, check local database
        if "timeout" in err_msg.lower() or "connect" in err_msg.lower() or "endpoint" in err_msg.lower():
            print("Cognito server connection timed out. Falling back to offline check...")
            try:
                profile_res = call_supabase_api("user_profiles", query_params={"email": f"eq.{email}"})
                if profile_res and len(profile_res) > 0:
                    role = profile_res[0].get('role')
                    ...
                    # Accept password as ThrustVault123! or any password in offline mode
                    user_id = profile_res[0].get('id', 'mock_admin_uid')
                    session['email'] = email
                    session['role'] = role
                    session['uid'] = user_id
...
# ---------------------------------------------------------------------------
# Guest APIs (GET only, needed for dashboard populations)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Intern APIs (re-used for database manipulations)
# ---------------------------------------------------------------------------
